  ...
  - [Deployment Scenarios](#h-6BCD4BE2)
    - [Defining a JupyterHub Cluster](#h-487A6DED)
      ...
      - [secrets.yaml](#h-B14026AE)
        - [Container Registry Authentication](#h-A5FD307F)
        - [Security Secrets](#h-E16434F2)
    ...
    - [Launching a Jetstream2 Openstack Magnum Cluster](#h-C18A5351)
      ...
      - [Launching the JupyterHub](#h-C9998D99)
        ...
        - [Actually Launching the JupyterHub](#h-24179B9A)
...
The `imagePullSecret` section defines credentials used by Kubernetes to pull container images from a Docker registry. It enables authentication by creating a secret with the provided registry URL, `username`, and `password` (which should be a [personal access token obtained via the registry](https://docs.docker.com/security/for-developers/access-tokens/)).
...
You will have to set values for the `cookieSecret` and `secretToken`. These are randomly assigned values to ensure secure communication with and within the JupyterHub. They can be created with the `openssl rand -hex 32` command and should be unique for every JupyterHub server.
...
In your favorite editor (e.g., `vi`, `nano`), examine the cluster creation script `create_cluster.sh`. You may choose to alter some of the parameters that are described in the following table depending on your objectives. Note that we will be creating an additional `nodegroup` to host the JupyterLab single-user pods which is why `N_NODES` and `max_node_count` are set to `1`.

| Variable / Option | Default Value | Recommended | Description                                    |
|----------------- |------------- |----------- |---------------------------------------------- |
| FLAVOR            | m3.small      | m3.quad     | VM size for worker nodes (CPU/RAM)             |
| N\_NODES          | 1             | 1           | Number of worker nodes                         |
| N\_MASTER         | 3             | 1           | Number of control plane nodes                  |
| min\_node\_count  | 1             | 1           | Minimum node count when autoscaling is enabled |
| max\_node\_count  | 5             | 1           | Maximum node count when autoscaling is enabled |

At this point, you are ready to launch your cluster:
...
The following command will create a new auto-scaling nodegroup named `mediums` with a maximum of 8 nodes to accommodate the single-user pods:
...
An ingress resource allows HTTP traffic into the cluster. Install it on the `default-worker` node with:
...
After a short while, you should see the ingress service resource:

```sh
kubectl get svc -n ingress-nginx
```

Set the `IP` that was just provided with:
...
##### Link Domain Name to IP Address

Create an "A record", i.e., a subdomain name, for users to access the cluster via a browser.
...
This command should return something like: `<project id>.projects.jetstream-cloud.org.`

Next link the domain name to the IP address:

```sh
openstack recordset create <project id>.projects.jetstream-cloud.org. $K8S_CLUSTER_NAME --type A --record $IP --ttl 3600
```

To check the status of that command with the `id` that was just supplied:

```sh
openstack recordset show <project id>.projects.jetstream-cloud.org. <id>
```

You should now be able to access at `$K8S_CLUSTER_NAME.<project id>.projects.jetstream-cloud.org`. At this stage, you'll see a blank nginx page, since we haven't set up any services, such as JupyterHub, yet.
...
A certificate from LetsEncrypt allows secure connections via HTTPS. To enable that, `cd` to `/jupyterhub-deploy-kubernetes-jetstream/setup_https`. Edit `https_cluster_issuer.yaml` to include an email address for communication with LetsEncrypt.

Deploy the `cert-manager` pods by applying the manifest:

```sh
kubectl apply -f https://github.com/cert-manager/cert-manager/releases/download/v1.16.2/cert-manager.yaml
```

Now, apply the cluster issuer manifest:
...
##### Actually Launching the JupyterHub

First, `cd` to `~/jupyterhub-deploy-kubernetes-jetstream` run the following command to install JupyterHub:

```sh
bash configure_helm_jupyterhub.sh
```

Next install JupyterHub. This action will install the JupyterHub according to the `secrets.yaml` we developed earlier.

```sh
bash install_jhub.sh
```

Navigate to your cluster's URL and verify that the connection is secure (HTTPS in working) and that you can login with your GitHub credentials.
